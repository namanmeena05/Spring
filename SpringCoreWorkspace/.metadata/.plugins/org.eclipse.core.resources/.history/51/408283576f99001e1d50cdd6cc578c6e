...
public class UserRegistrationCtl {
	...
	public String display(@ModelAttribute("form") UserRegistrationForm form, Model m) {
		/*
		 * UserDTO dto = new UserDTO(); dto.setId(form.getId());
		 * dto.setFirstName(form.getFirstName()); dto.setLastName(form.getLastName());
		 * dto.setLogin(form.getLogin()); dto.setPassword(form.getPassword());
		 * dto.setDob(DataUtility.stringToDate(form.getDob())); service.add(dto);
		 */
